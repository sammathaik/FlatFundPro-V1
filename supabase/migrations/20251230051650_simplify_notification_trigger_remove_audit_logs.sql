/*
  # Simplify Notification Trigger - Remove Audit Log Dependencies

  ## Overview
  The notification trigger was not firing due to audit_logs table permission issues.
  This migration simplifies the trigger to focus solely on creating notifications.

  ## Changes
  1. Remove all audit_logs insertions from trigger function
  2. Keep core notification creation logic
  3. Improve error handling to not silently fail

  ## Purpose
  - Focus on core notification creation functionality
  - Remove dependencies that could cause silent failures
  - Improve debugging by removing complex error handling
*/

-- Recreate the notification trigger function without audit_logs
...
'Generates WhatsApp notification preview for payment submissions. Simplified version without audit logging to prevent trigger failures from blocking payment flow.';
