/*
  # Fix Approval Notification Trigger - Case Sensitivity Issue
  
  1. Problem
    - Trigger checks for lowercase 'approved' but database uses 'Approved'
    - This causes approval notifications to never send
    - No communication audit logs are created for approvals
  
  2. Solution
    - Update trigger to check for 'Approved' (capital A)
    - Make comparison case-insensitive using LOWER()
  
  3. Impact
    - Approval emails will now send correctly
    - Communication audit trail will now log approval notifications
*/
    ...
    BEGIN
      ...
      SELECT net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'approved_by_user_id', NEW.approved_by
        )
      ) INTO v_request_id;
...
'Sends email and WhatsApp notifications when payment is approved. Uses case-insensitive status check. Logs all communications to unified audit trail. Non-blocking - never fails payment approval.';
