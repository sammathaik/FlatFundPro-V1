/*
  # Fix Admin Manual Payment - Correct ON CONFLICT Constraint

  ## Overview
  Fixes the ON CONFLICT clause in create_admin_manual_payment function to match
  the actual unique constraint on flat_email_mappings table.

  ## Issue
  The function was using ON CONFLICT (apartment_id, block_id, flat_id, occupant_type)
  but the actual constraint is UNIQUE (apartment_id, block_id, flat_id)

  ## Fix
  Update the ON CONFLICT clause to match the actual constraint.
*/
...
BEGIN
  ...
  -- FIXED: Use correct constraint (apartment_id, block_id, flat_id)
