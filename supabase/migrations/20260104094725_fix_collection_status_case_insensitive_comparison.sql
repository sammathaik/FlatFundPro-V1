/*
  # Fix Collection Status Summary - Case-Insensitive Status Comparison
  
  1. Problem
    - Function checks for 'approved' (lowercase) but database has 'Approved' (capital A)
    - This causes approved payments to not show in the status grid
    
  2. Changes
    - Make status comparison case-insensitive using LOWER()
    - This will match 'Approved', 'approved', 'APPROVED', etc.
*/
...
BEGIN
  ...
  payment_summary AS (
    SELECT
      ...
      COUNT(CASE WHEN LOWER(ps.status) = 'pending' THEN 1 END) as pending_count,
...
COMMENT ON FUNCTION get_collection_status_summary IS 'Returns payment status summary for a specific collection, grouped by flat. Uses case-insensitive status comparison to match Approved, approved, APPROVED, etc.';
