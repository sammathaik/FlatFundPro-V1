/*
  ...
  1. Issue
    - Functions were using `collection_type` but actual column is `collection_name`
    - `payment_submissions` uses `expected_collection_id` foreign key, not direct `collection_type`
  
  2. Changes
    - Update all analytics functions to use correct column names
    - Join on `expected_collection_id` to get `collection_name`
*/

-- =====================================================================
-- 1. Executive Summary with Correct Column Names
-- =====================================================================
...
BEGIN
  ...
  WITH collection_breakdown AS (
    ...
    GROUP BY ec.id, ec.collection_name, ec.amount_due
  ),
  totals AS (
    SELECT
    ...
    'recentPayments', (
      SELECT COALESCE(json_agg(
        json_build_object(
          ...
          'collectionType', ec.collection_name,
      ...
      LEFT JOIN expected_collections ec ON recent.expected_collection_id = ec.id
    ),
    'topDefaulters', (
      SELECT COALESCE(json_agg(
        json_build_object(
      ...
      FROM (
        WITH flat_expected AS (
          SELECT 
            ...
            array_agg(DISTINCT ec.collection_name ORDER BY ec.collection_name) as collection_types
          FROM flat_numbers fn
          JOIN buildings_blocks_phases bbp ON fn.block_id = bbp.id
...
-- =====================================================================
-- 2. Collection Performance with Correct Column Names
-- =====================================================================
  ...
  SELECT json_build_object(
    ...
    'collectionTypePerformance', (
      ...
      FROM (
        ...
        AND ec.is_active = true
        AND ec.due_date BETWEEN p_start_date AND p_end_date
        GROUP BY ec.id, ec.collection_name
      ) type_data
    ),
    'monthlyTrends', (
      SELECT COALESCE(json_agg(
...
-- =====================================================================
-- 3. Collection Efficiency with Correct Column Names
-- =====================================================================
