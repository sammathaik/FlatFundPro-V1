/*
  # Admin Manual Payment Entry System

  ## Overview
  This migration creates the system for Admins to manually create trusted payment records
  when they receive payment details offline (call, email, WhatsApp, etc.).

  ## Key Features
  - Admin-only function to create trusted payment records
  - Bypasses OCR, AI classification, and fraud detection
  - Automatically marked as "Approved"
  - Creates/updates flat-email mappings
  - Optionally triggers Email and WhatsApp notifications
  - Full audit trail logging
  - Calculates expected amounts with late fines

  ## Security
  - SECURITY DEFINER function restricted to authenticated admins only
  - Validates admin access to apartment
  - Validates all input fields
  - Prevents SQL injection through parameterized queries

  ## Important Notes
  - This is for TRUSTED, ADMIN-VERIFIED payments only
  - Does NOT run fraud detection or OCR
  - Status is always 'Approved' (not 'Received' or 'Reviewed')
  - Screenshot URL is set to placeholder indicating admin entry
  - Marked with special metadata to distinguish from occupant submissions
*/

-- Function to create admin manual payment entry
...
BEGIN
  ...
  SELECT
    collection_name,
    COALESCE(amount_per_flat, 0) as expected_amount,
    due_date,
    COALESCE(late_fine_per_day, 0) as daily_fine
  ...
  ON CONFLICT (apartment_id, block_id, flat_id)
  DO UPDATE SET
    email = lower(trim(p_email)),
    occupant_type = p_occupant_type,
    name = trim(p_occupant_name),
    mobile = p_mobile,
    whatsapp_optin = COALESCE(p_whatsapp_optin, false),
    updated_at = now();

  -- 6. CREATE payment submission record as APPROVED
  -- NOTE: No screenshot_url since this is admin-entered
  -- Using special marker to indicate admin manual entry
  INSERT INTO payment_submissions (
    ...
    payment_source,
    occupant_type,
    -- Admin entry specific fields
    platform,
    narration,
    other_text,
    -- Fraud detection NOT applied (admin trusted entry)
    fraud_score,
    is_fraud_flagged,
    fraud_checked_at,
  ...
  -- 7. LOG to communication_logs for email notification (if email provided)
  ...
  -- 8. LOG to communication_logs for WhatsApp notification (if opted in)
  ...
  -- 10. RETURN success with full details
  RETURN jsonb_build_object(
    'success', true,
...
GRANT EXECUTE ON FUNCTION create_admin_manual_payment TO authenticated;

COMMENT ON FUNCTION create_admin_manual_payment IS 'Admin-only function to create trusted payment records for offline payments. Bypasses OCR, AI, and fraud detection. Immediately marks as Approved.';
