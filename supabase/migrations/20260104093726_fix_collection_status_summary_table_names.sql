/*
  # Fix Collection Status Summary Function - Correct Table Names
  
  1. Problem
    - Function uses wrong table names: `flats` and `buildings`
    - Actual tables are: `flat_numbers` and `buildings_blocks_phases`
    
  2. Changes
    - Rewrite function to use correct table names
    - Update joins to match actual schema
    
  3. Schema Reference
    - flat_numbers: id, block_id, flat_number
    - buildings_blocks_phases: id, block_name, apartment_id
    - flat_email_mappings: flat_id, name, is_primary
    - payment_submissions: flat_id, expected_collection_id, payment_amount, status
*/
...
BEGIN
  ...
  WITH flat_info AS (
    ...
    LEFT JOIN flat_email_mappings fem ON fem.flat_id = fn.id AND fem.is_primary = true
    WHERE EXISTS (
...
-- Grant execution to both authenticated and anonymous users
...
COMMENT ON FUNCTION get_collection_status_summary IS 'Returns payment status summary for a specific collection, grouped by flat. Uses correct table names: flat_numbers and buildings_blocks_phases.';
