-- ============================================
-- SIMPLE DEBUG: Why Partial Instead of Paid?
-- ============================================
-- Run this to see the exact issue

-- For Meenakshi apartment, show one example calculation
SELECT 
  fn.flat_number,
  ec.payment_type,
  ec.quarter,
  ...
  STRING_AGG(ps.payment_amount::text, ', ') as individual_payments,
  -- Status
  CASE 
    WHEN (ec.amount_due + 
          CASE 
  ...
  END as status,
  -- Show the math
  COALESCE(SUM(ps.payment_amount), 0)::text || ' >= ' || 
  (ec.amount_due + 
   CASE 
     WHEN ec.daily_fine > 0 AND ec.due_date < CURRENT_DATE 
     THEN (CURRENT_DATE - ec.due_date)::integer * ec.daily_fine
    ...
    WHEN COALESCE(SUM(ps.payment_amount), 0) >= 
         (ec.amount_due + 
          CASE 
            ...
            ELSE 0
          END)
    THEN 'YES → Should be PAID'
    ELSE 'NO → Shows as PARTIAL (needs ' || 
         ((ec.amount_due + 
           CASE 
             WHEN ec.daily_fine > 0 AND ec.due_date < CURRENT_DATE 
             THEN (CURRENT_DATE - ec.due_date)::integer * ec.daily_fine
             ELSE 0
           END) - COALESCE(SUM(ps.payment_amount), 0))::text || ' more)'
  END as calculation
...
WHERE a.apartment_name ILIKE '%meenakshi%'
  ...
  AND COALESCE(SUM(ps.payment_amount), 0) > 0  -- Only show flats with payments
GROUP BY fn.id, fn.flat_number, ec.id, ec.payment_type, ec.quarter, 
