...
interface CollectionEfficiency {
  ...
  averageDelay: number;
  onTimeRate: number;
  collectionTypeEfficiency: TypeEfficiency[];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Analyze payment timeliness and collection patterns for {data.flatCount} flats</p>
        </div>
      ...
      <div id="collection-efficiency-report" className="space-y-6">
        ...
        {/* Collection Type Efficiency */}
        {data.collectionTypeEfficiency && data.collectionTypeEfficiency.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Efficiency by Collection Type</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {data.collectionTypeEfficiency.map((type, index) => (
                <div key={index} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-900">{type.totalPayments}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">On-Time:</span>
                      <span className="font-semibold text-green-700">{type.onTimePayments}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">On-Time Rate:</span>
                      <span className={`font-bold ${
                        type.onTimeRate >= 80 ? 'text-green-600' :
                        type.onTimeRate >= 60 ? 'text-yellow-600' : 'text-red-600'
                      }`}>
                        {formatPercentage(type.onTimeRate)}
                      </span>
                    </div>
                  </div>
                  <div className="mt-3 w-full bg-gray-300 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        type.onTimeRate >= 80 ? 'bg-green-600' :
                        type.onTimeRate >= 60 ? 'bg-yellow-600' : 'bg-red-600'
                      }`}
                      style={{ width: `${Math.min(type.onTimeRate, 100)}%` }}
                    />
                  </div>
                </div>
              ))}
